...
import { stackTopicName } from '../cloudwatch-stack';
...
export async function handler(event: lambda.DynamoDBStreamEvent | any, context: lambda.Context | any) {
  ...
  if (!event.Records || event.Records.length !== 1 || !event.Records[0].dynamodb) {
    ...
    throw Error('event object not valid!');
  ...
  if (newImage) {
    ...
    const createStackParams: AWS.CloudFormation.Types.CreateStackInput = {
      ...
      NotificationARNs: [`arn:aws:sns:${process.env.AWS_REGION}:${accountId}:${stackTopicName}`],
      Parameters: [{
  ...
  throw Error('event not valid! Exactly one record allowed!');
  // const putMetricDataParam: AWS.CloudWatch.Types.PutMetricDataInput = {
  //   Namespace: 'Scheduler',
  //   MetricData: [{
